...
<head>
    ...
    <title>Lista Estudantes Classe | Gestão Escolar</title>
...
fixed; background-size: cover; margin: 20px; background-image: url({{ public_path('assets/images/'. $escola->logotipo_documentos ?? '') }});'>
    ...
    <div id="header">
      ...
      <div class="texto-header">
        <div>
          ...
          <h1 class="fs-5"><strong>LISTA DOS ESTUDANTES DAS {{ $classe->classes }} {{ $anolectivo->ano }}</strong></h1>
          @else
    ...
    <div class="col-12 table-responsive">
      @if ($matriculas)
        <table  style="width: 100%" class="table table-stripeds" style="margin-top: 170px">
          <thead>
          <tr>
            <th>Cod</th>
            <th>Estudante</th>
          ...
          <tbody>
              @foreach ($matriculas as $matricula)
                ...
                <tr>
                    ...
                    <td>{{ $cursos->curso }}</td>
                    <td class="bg-dark">{{ $classes->classes }}</td>
                    <td>{{ $turnos->turno }}</td>
                </tr>
              @endforeach
