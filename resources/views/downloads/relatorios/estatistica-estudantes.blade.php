...
fixed; background-size: cover; margin: 20px; background-image: url({{ public_path('assets/images/'. $escola->logotipo_documentos ?? '') }});'>
    ...
    <div class="row" style="margin-top: 170px">
       <table>
            <thead>
                 <tr>
                    <th colspan="2">CURSO: @if($curso) {{ $curso->curso }} @else GERAL @endif</th>
                    <th colspan="2">CLASSES: @if ($classe) {{ $classe->classes }} @else GERAL @endif</th>
                    <th colspan="2">TURNOS: @if ($turno) {{ $turno->turno }} @else GERAL @endif</th>
                    <th colspan="2">PROVÍNCIA: @if ($provincia) {{ $provincia->name }} @else GERAL @endif</th>
                    <th>ESTADO: @if ($estado) {{ $estado == 'confirmado' ? 'Admitidos' : 'Não Admitidos'  }} @else GERAL @endif</th>
                    <th>ANO: @if ($ano) {{ $ano->ano }} @else GERAL @endif</th>
                </tr>
                <tr>
                    <th>Nº</th>
                ...
                @if (count($estudantes) != 0)
                    @foreach ($estudantes as $key => $item)
                        <tr>
                            ...
                            <td>{{ $item->status_matricula == 'confirmado' ? 'Admitido' : 'Não Admitido' }}</td>
                            <td>{{ $item->genero }}</td>
                            <td>{{ $item->classes }}</td>
                            <td>{{ $item->curso }}</td>
                            <td>{{ $item->turno }}</td>
            ...
            <tfoot>
                <tr>
                    <th colspan="10" style="text-align: left">TOTAL REGISTOS: {{ count($estudantes) }}</th>
                </tr>
            </tfoot>
       </table>
    </div>
