...
<body class="hold-transition sidebar-mini">
  ...
  <div class="row" style="margin-top: 170px">
    <table>
      <thead>
        <tr>
          <th style="text-align: left;" colspan="3">INSTITUIÇÃO:</th>
          <th style="text-align: left;text-transform: uppercase" colspan="7">{{ $instituicao ? $instituicao->nome : "TODOS" }}</th>
        </tr>
        <tr>
          <th style="text-align: left;" colspan="3">BOLSA:</th>
          <th style="text-align: left;text-transform: uppercase" colspan="7">{{ $estagiario ? $estagiario->nome : "TODOS" }}</th>
        </tr>
        
        <tr>
          <th colspan="3" style="text-align: left">TOTAL REGISTROS:</th>
          <th colspan="7" style="text-align: left">{{ count($estagiarios) }}</th>
        </tr>
        ...
        <tr>
            ...
            <th style="text-align: left">Estagio</th>
            <th style="text-align: left">Instituição</th>
            <th style="text-align: left">Tipo Instituição</th>
      ...
      <tbody class="text-center">
        @foreach ($estagiarios as $key => $item)
            <tr>
                ...
                <td style="text-align: left">{{ $item->estagio->nome ?? '' }}</td>
                <td style="text-align: left">{{ $item->instituicao->nome ?? '' }}</td>
                <td style="text-align: left">{{ $item->instituicao->tipo ?? '' }}</td>
                <td style="text-align: left">{{ $item->data_inicio ?? '' }}</td>
                <td style="text-align: left">{{ $item->data_final ?? '' }}</td>
