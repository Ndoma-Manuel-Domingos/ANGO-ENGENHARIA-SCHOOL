...
fixed; background-size: cover; margin: 20px; background-image: url({{ public_path('assets/images/'. $escola->logotipo_documentos ?? '') }});'>
    ...
    <div class="row" style="margin-top: 170px">
       <table>
            <thead>
                <tr>
                  <th colspan="5" style="text-align: left">PROVÍNCIAS: @if ($provincia) {{ $provincia->name }}@else Geral @endif</th>
                  <th style="text-align: left" colspan="5">GENERO: @if ($genero)  {{ $genero }}@else Geral @endif</th>
                </tr>
                <tr>
                    <th>Nº</th>
                    <th>Nome</th>
            ...
            <tbody class="text-center">
                @if (count($professores) != 0)
                    @foreach ($professores as $item)
                        <tr>
                            <td class="text-secondary">{{ $item->id }}</a></td>
                            <td class="text-secondary">{{ $item->nome }} {{ $item->sobre_nome }}</a></td>
                            ...
                            <td>{{ $item->total_escola($item->id) }}</td>
                        </tr>    
                    @endforeach
                @endif
            </tbody>
            ...
            <tfoot>
              <tr>
                <th colspan="10" style="text-align: left">TOTAL DE REGISTROS: {{ count($professores) }}</th>
              </tr>
            </tfoot>
       </table>
    </div>
