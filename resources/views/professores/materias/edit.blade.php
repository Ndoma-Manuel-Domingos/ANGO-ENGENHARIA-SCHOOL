...
<div class="content">
  ...
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">Editar Matéria</h1>
        </div>
        <div class="col-sm-6">
  ...
  <section class="content">
    <div class="container-fluid">
      ...
      <div class="row">
        <div class="col-12 col-md-12">
          <form action="{{ route('portal-professor-minhas-materias.update', $materia->id) }}" method="post" enctype="multipart/form-data" id="formulario_cadastro_professor">
            @csrf
            @method('put')
            <div class="card">
                    ...
                    <div class="row">
                      ...
                      <div class="form-group col-12 col-md-3">
                        ...
                        <select type="text" class="form-control select2" placeholder="Escolas" name="escola_id" id="escola_id">
                          ...
                          @foreach ($escolas as $item)
                            <option value="{{ $item->id }}" {{ $item->id ==   $materia->shcools_id ? 'selected' : '' }}>{{ $item->nome }}</option>  
                          @endforeach
                        </select>
                      </div>
                      ...
                      <div class="form-group col-12 col-md-3">
                        ...
                        <select type="text" class="form-control select2" placeholder="Turmas" name="turmas_id" id="turmas_id">
                          ...
                          @foreach ($turmas as $item)
                            <option value="{{ $item->idTurma }}" {{ $item->idTurma ==   $materia->turmas_id ? 'selected' : '' }}>{{ $item->turma }}</option>  
                          @endforeach
                        </select>
                      </div>
                      ...
                      <div class="form-group col-12 col-md-3">
                        ...
                        <select type="text" class="form-control select2" placeholder="Disciplinas" name="disciplinas_id" id="disciplinas_id">
                          <option value="">TODAS</option>
                          @foreach ($turmas as $item)
                            <option value="{{ $item->idDis }}" {{ $item->idDis ==   $materia->disciplinas_id ? 'selected' : '' }}>{{ $item->disciplina }}</option>  
                          @endforeach
                        </select>
                      </div>
                      ...
                      <div class="form-group col-12 col-md-3">
                        ...
                        <select type="text" class="form-control select2" placeholder="Ano Lectivos" name="ano_lectivos_id" id="ano_lectivos_id">
                          ...
                          @foreach ($anos_lectivos as $item)
                            <option value="{{ $item->id }}" {{ $item->id == $materia->ano_lectivos_id ? 'selected' : '' }}>{{ $item->ano }}</option>  
                          @endforeach
                        </select>
                      </div>
                      ...
                      <div class="form-group col-12 col-md-3">
                        ...
                        <input type="date" class="form-control" name="data_limite" placeholder="Informe o titulo da Materia" value="{{ $materia->data_limite }}">
                      </div>
                      
                      <div class="form-group col-12 col-md-3">
                        <label for="" class="form-label">Primeiro Documento</label>
                        <input type="file" class="form-control" name="documento1" placeholder="Informe o documento1">
                        <input type="hidden" name="documento1_guardado" value="{{ $materia->documento1 }}">
                      </div>
                      
                      <div class="form-group col-12 col-md-3">
                        <label for="" class="form-label">Segundo Documento</label>
                        <input type="file" class="form-control" name="documento2" placeholder="Informe o documento2">
                        <input type="hidden" name="documento2_guardado" value="{{ $materia->documento2 }}">
                      </div>
                      
                      <div class="form-group col-12 col-md-3">
                        <label for="" class="form-label">Terceiro Documento</label>
                        <input type="file" class="form-control" name="documento3" placeholder="Informe o documento3">
                        <input type="hidden" name="documento3_guardado" value="{{ $materia->documento3 }}">
                      </div>
                      
                      <div class="form-group col-12 col-md-6">
                        <label for="" class="form-label">Titulo</label>
                        <textarea cols="30" rows="5" class="form-control" name="titulo" placeholder="Informe o titulo da Materia">{{ $materia->titulo }}</textarea>
                      </div>
                      
                      
                      <div class="form-group col-12 col-md-6">
                        <label for="" class="form-label">Descrição</label>
                        <textarea cols="30" rows="5" class="form-control" name="descricao" placeholder="Informe o titulo da Materia">{{ $materia->descricao }}</textarea>
                      </div>
