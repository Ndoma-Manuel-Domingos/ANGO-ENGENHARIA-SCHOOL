    ...
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-12">
                ...
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('prof.minhas-solicitacoes') }}" method="get" id="formulario">
                            <div class="row">
                ...
                <div class="card">
                    <div class="card-header">
                        <a href="{{ route('prof.solicitacoes-processo-store') }}" class="btn btn-primary mx-2 float-end">Fazer nova solicilição</a>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body table-responsive">
                        <table id="tableSolicitacoes" style="width: 100%" class="table  table-bordered table-striped  ">
                            <thead>
                                <tr>
                                    <th>Nº</th>
                                    <th>Professor</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>Solicitação de {{ $item->solicitacao ?? '' }}</td>
                                    <td>{{ $item->status == 1 ? 'Respondido' : 'Não Respondido' }}</td>
                                    <td>{{ $item->user->nome ?? 'N/A' }}</td>
                                    <td>{{ date("d-m-Y", strtotime($item->created_at ?? '')) }} ÁS {{ date("H:i:s", strtotime($item->created_at ?? '')) }}</td>
                                    {{-- <td>{{ $item->instituicao1->nome ?? 'N/A' }}</td> --}}
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            <div class="dropdown-menu" role="menu">
                                                <a href="{{ route('prof.detalhes-minhas-solicitacoes', $item->id) }}" class="dropdown-item"><i class="fa fa-eye"></i> Visualizar</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
