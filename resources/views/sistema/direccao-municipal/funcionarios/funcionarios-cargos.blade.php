...
<div class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1 class="m-0 text-dark">Funcionários Por Cargos <span> de <span class="text-dark">{{ $cargo->cargo ?? '' }}</span></span> </h1>
      </div><!-- /.col -->
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item"><a href="{{ route('web.funcionarios-municipal-controlo') }}">Voltar</a></li>
  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-12 col-md-12">
        <form action="{{ route('web.funcionarios-municipal-cargos') }}" method="get">
          @csrf
          <div class="card">
            <div class="card-body">
              <div class="row">
                ...
                <div class="form-group mb-3 col-md-3 col-12">
                  ...
                  <select name="cargo_id" id="cargo_id" class="form-control cargo_id select2">
                    ...
                    <option value="{{ $item->id }}" {{ $requests['cargo_id']==$item->id ? 'selected' : '' }}>{{ $item->cargo }}</option>
                    @endforeach
                  </select>
                  @error('cargo_id')
                  <span class="text-danger error-text">{{ $message }}</span>
                ...
                <div class="form-group mb-3 col-md-3 col-12">
                  ...
                  <select name="tempo_trabalho" id="tempo_trabalho" class="form-control tempo_trabalho select2">
                    <option value="">Todos</option>
                      @for ($i = 0; $i < 61; $i++)
                        <option value="{{ $i }}" {{ $requests['tempo_trabalho']==$i ? 'selected' : '' }}>{{ $i }} Anos</option>
                      @endfor
                    </option>
                  </select>
        ...
        <div class="card">
          <div class="card-header">
            <h6>Total registro : {{ count($funcionarios) }}
                <a href="{{ route('funcionarios-imprmir-cargos-municipal-pdf', ['instituicao' => 3,  'status' => $requests['status'] ?? "", 'genero' => $requests['genero'] ?? "", 'cargo_id' => $requests['cargo_id'] ?? "", 'tempo_trabalho' => $requests['tempo_trabalho'] ?? ""]) }}" target="_blink"
                  class="btn-danger btn float-end mx-1">Imprimir PDF</a> 
                
                <a href="{{ route('funcionarios-imprmir-cargos-municipal-excel', ['instituicao' => 3,  'status' => $requests['status'] ?? "", 'genero' => $requests['genero'] ?? "", 'cargo_id' => $requests['cargo_id'] ?? "", 'tempo_trabalho' => $requests['tempo_trabalho'] ?? ""]) }}" target="_blink"
          ...
          <div class="card-body">
            <table id="carregarTabelaFuncionarios" style="width: 100%"
              ...
              <tbody>
                ...
                <tr>
                  <td>{{ $key + 1 }}</td>
                  <td><a href="{{ route('web.funcionarios-municipal-show', $item->id) }}">{{ $item->nome ?? '' }} {{ $item->sobre_nome ?? '' }}</a></td>
                  ...
                  <td>
                    <div class="btn-group">
                      ...
                      <div class="dropdown-menu" role="menu">
                        ...
                        <a href="{{ route('web.funcionarios-municipal-show', $item->id) }}" title="excluir Funcionarios"class="delete_funcionarios dropdown-item"><i class="fa fa-eye"></i> Visualizar</a>
                        @endif

                        @if (Auth::user()->can('update: professores'))
                        <a href="{{ route('web.funcionarios-municipal-edit', $item->id) }}" title="Editar Funcionarios" class="dropdown-item"><i class="fa fa-edit"></i> Editar </a>
                        @endif

                        @if (Auth::user()->can('delete: professores'))
                        <a href="{{ route('web.funcionarios-municipal-destroy', $item->id) }}"
                          title="excluir Funcionarios" id="{{ $item->id }}" class="delete_funcionarios dropdown-item"><i class="fa fa-trash"></i> Excluir</a>
                        @endif
