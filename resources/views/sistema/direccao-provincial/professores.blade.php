      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-3">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ route('web.informacao-escola-provincial', Crypt::encrypt($escola->id)) }}">Voltar à escola</a></li>
                                ...
                                <tr>
                                    <td>
                                      @if (Auth::user()->can('read: professores'))
                                        <a href="{{ route('app.informacao-professores-provincial', Crypt::encrypt($item->funcionario->id)) }}" class="text-secondary">{{ $item->funcionario->id }}</a>
                                      @else
                                        {{ $item->funcionario->id }}
                                      @endif
                                    </td>
                                    ...
                                    <td>
                                      @if (Auth::user()->can('read: professores'))
                                        <a href="{{ route('app.informacao-professores-provincial', Crypt::encrypt($item->funcionario->id)) }}" class="text-secondary">{{ $item->funcionario->nome }} {{ $item->funcionario->sobre_nome }}</a>
                                      @else
                                    ...
                                    <td>
                                      @if (Auth::user()->can('read: professores'))
                                        <a href="{{ route('app.informacao-professores-provincial', Crypt::encrypt($item->funcionario->id)) }}" title="Visualizar Professor" id="{{ $item->funcionario->id }}" class="btn btn-primary"><i class="fa fa-eye"></i> Visualizar </a>
                                      @endif
                                    </td>
                                </tr>    
                            @endforeach
