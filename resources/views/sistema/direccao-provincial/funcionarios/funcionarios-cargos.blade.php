  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-12 col-md-12">
        <form action="{{ route('web.funcionarios-provincial-cargos') }}" method="get">
          @csrf
          <div class="card">
            <div class="card-body">
              <div class="row">
        ...
        <div class="card">
          <div class="card-header">
            <h6>Total registro : {{ count($funcionarios) }}
                {{-- <a href="{{ route('funcionarios-imprmir-cargos-provincial-pdf', ['instituicao' => 3,  'status' => $requests['status'] ?? "", 'genero' => $requests['genero'] ?? "", 'cargo_id' => $requests['cargo_id'] ?? "", 'tempo_trabalho' => $requests['tempo_trabalho'] ?? ""]) }}" target="_blink"
                  class="btn-danger btn float-end mx-1">Imprimir PDF</a> 
                
                <a href="{{ route('funcionarios-imprmir-cargos-provincial-excel', ['instituicao' => 3,  'status' => $requests['status'] ?? "", 'genero' => $requests['genero'] ?? "", 'cargo_id' => $requests['cargo_id'] ?? "", 'tempo_trabalho' => $requests['tempo_trabalho'] ?? ""]) }}" target="_blink"
          ...
          <div class="card-body">
            <table id="carregarTabelaFuncionarios" style="width: 100%"
              ...
              <tbody>
                ...
                <tr>
                  <td>{{ $key + 1 }}</td>
                  <td><a href="{{ route('web.funcionarios-provincial-show', Crypt::encrypt($item->id)) }}">{{ $item->nome ?? '' }} {{ $item->sobre_nome ?? '' }}</a></td>
                  ...
                  <td>
                    <div class="btn-group">
                      ...
                      <div class="dropdown-menu" role="menu">
                        ...
                        <a href="{{ route('web.funcionarios-provincial-show', Crypt::encrypt($item->id)) }}" title="excluir Funcionarios"class="delete_funcionarios dropdown-item"><i class="fa fa-eye"></i> Visualizar</a>
                        @endif
