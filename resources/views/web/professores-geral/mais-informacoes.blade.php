                  ...
                  <div class="form-horizontal">
                    ...
                    @if (Auth::user()->can('read: distribuicao de professor') || Auth::user()->can('read: transeferencia professor') || Auth::user()->can('read: professores'))
                      @if (count($infor_escola) > 0)
                        ...
                        <div class="form-group row">
                          <label for="inputName2" class="col-sm-2 col-form-label"><a href="{{ route('app.informacao-turma-professores', [$professor->id, $item->id]) }}">Mais informação</a></label>
                          <div class="col-sm-10">
                            <div class="form-control"><a href="{{ route('web.informacao-escola', $item->id) }}" class="text-secondary">{{ $item->nome }} - <span class="text-danger">Província de {{ $item->provincia->nome }}</span></a></div>
                          </div>
                        </div>    
                        @endforeach
                      @endif
            ...
            <div class="card-body">
              ...
              @if (Auth::user()->can('update: estado'))
                @if ($professor->status == "desactivo")
                  <a href="{{ route('app.estado-candidaturas-professores', $professor->id) }}" class="btn btn-success">Activar Candidatura</a>    
                @else
                  <a href="{{ route('app.estado-candidaturas-professores', $professor->id) }}" class="btn btn-danger">Desactivar Candidatura</a>  
                @endif
              @endif
              
              @if (Auth::user()->can('create: distribuicao de professor'))
              <a href="{{ route('app.Dispanho-professores-index', $professor->id) }}" class="btn btn-primary">Distribuir para escolas</a>  
              @endif
              
              @if (Auth::user()->can('create: transeferencia professor'))
              <a href="{{ route('web.transferencia-escola-professores', $professor->id) }}" class="btn btn-primary">Aplicar Transferência</a>  
              @endif
            </div>
          </div>
