...
<div class="p-4 lg:p-8 space-y-8">
    <nav class="flex items-center gap-2 text-xs text-slate-500 mb-6">
        ...
        <span>Descontos</span>
        <span class="material-symbols-outlined text-[14px]">chevron_right</span>
        <span class="text-primary font-semibold">gestão de estudantes</span>
    </nav>
    
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Estudantes com descontos</h1>
    ...
    <div class="bg-white dark:bg-sidebar-dark rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex flex-col lg:flex-row gap-4 lg:items-center justify-between">
            <div class="flex flex-wrap gap-3">
                ...
                <select id="ano_lectivos_id" class="h-10 px-3 bg-slate-50 dark:bg-slate-800 border-none rounded-lg text-sm text-slate-600 dark:text-slate-300 focus:ring-2 focus:ring-primary min-w-[150px]">
                ...
                <select id="descontos_id" class="h-10 px-3 bg-slate-50 dark:bg-slate-800 border-none rounded-lg text-sm text-slate-600 dark:text-slate-300 focus:ring-2 focus:ring-primary min-w-[150px]">
                    <option value="">Descontos</option>
                    @foreach ($descontos as $item)
                    <option value="{{ $item->id }}">{{ $item->nome }} - {{ $item->desconto }}%</option>
                    @endforeach
                </select>
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-slate-800/30 border-b border-slate-100 dark:border-slate-800">
                        <th class="px-6 py-4 text-xs font-bold uppercase text-slate-500 tracking-wider">Id</th>
                        <th class="px-6 py-4 text-xs font-bold uppercase text-slate-500 tracking-wider">Nº Processo</th>
                        ...
                        <th class="px-6 py-4 text-xs font-bold uppercase text-slate-500 tracking-wider">Taxa</th>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div
                ...
                <div class="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-800">
                    <h3 class="text-xl font-bold text-slate-900 dark:text-white" id="modalTitle">Atribuir Desconto</h3>
                ...
                <form class="p-6 space-y-5" action="{{ route('estudantes-descontos.store') }}" method="POST">
                    @csrf
                    <div>
                        <div class="col-span-2 md:col-span-1">
                            <label for="estudante_id" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Estudante</label>
                    ...
                    <div>
                        <div class="col-span-2 md:col-span-1">
                            <label for="desconto_id" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Desconto</label>
                            <select id="desconto_id" name="desconto_id" style="width: 100%;" class="select2 w-full h-11 px-4 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary focus:border-primary transition-all">
                                <option value="">Todos</option>
                                @foreach ($descontos as $item)
                                <option value="{{ $item->id }}">{{ $item->nome }} - {{ $item->desconto }}%</option>
                                @endforeach
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="col-span-2 md:col-span-1">
                            <label for="ano_lectivo_id" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Ano Lectiv</label>
                    ...
                    <div class="flex items-center justify-end gap-3 pt-8">
                        ...
                        @if (Auth::user()->can('create: desconto'))
...
@section('scripts')
    <script>
        ...
        $(document).on('click', '.delete-record', function(e) {
            ...
            , }).then((result) => {
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: `{{ route('estudantes-descontos.destroy', ':id') }}`.replace(':id', recordId), 
                        method: 'DELETE', 
                        data: {
        ...
        function edit(id) {
            $.ajax({
                url: `/estudantes-descontos/${id}/edit`,
                type: "GET",
                dataType: "json",
                beforeSend: function () {
                ...
                success: function (data) {
                    ...
                    $("#data_id").val(data.id);
                    $("#estudante_id").val(data.estudante_id).trigger('change');
                    $("#ano_lectivo_id").val(data.ano_lectivos_id).trigger('change');
                    $("#desconto_id").val(data.desconto_id).trigger('change');
                    $("#status").val(data.status);
        
                    $("#modalTitle").text("Editar desconto");
                    $("#botao_submiter").text("Editar desconto");
                ...
                error: function (xhr) {
                    ...
                    let message = "Erro ao carregar dados da desconto";
        
                    if (xhr.status === 404) {
                        message = "Desconto não encontrada";
                            ...
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors">
                                <td class="px-6 py-5">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="font-bold text-slate-500 dark:text-white">${s.id}</p>
                                            <p class="text-xs text-slate-500">${s.id}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-5">
                                    <p class="font-bold text-slate-500 dark:text-white">${s.estudante.numero_processo}</p>
                                </td>
                                <td class="px-6 py-5">
                                ...
                                <td class="px-6 py-5">
                                    <p class="font-bold text-slate-500 dark:text-white">${idade(s.estudante.nascimento)}</p>
                                </td>
                                <td class="px-6 py-5">
                                    <p class="font-bold text-slate-500 dark:text-white">${s.desconto.nome}</p>
                                </td>
                                <td class="px-6 py-5">
                                    <p class="font-bold text-slate-500 dark:text-white">${s.desconto.desconto}%</p>
                                </td>
                                <td class="px-6 py-5">
        ...
        function exportData(documentType) {
            ...
            $.ajax({
                type: "GET",
                url: `/estudantes-descontos-export`, // rota que vai gerar o arquivo
                data: data,
                xhrFields: {
                ...
                success: function(blob, status, xhr) {
                    ...
                    let filename = documentType === 'excel' ? 'estudantes-descontos.xlsx' : 'estudantes-descontos.pdf';
