...
<div class="p-4 lg:p-8 space-y-8">
    <nav class="flex items-center gap-2 text-xs text-slate-500 mb-6">
        ...
        <span>Descontos</span>
        <span class="material-symbols-outlined text-[14px]">chevron_right</span>
        <span class="text-primary font-semibold">gestão de tipos de descontos</span>
    </nav>
    
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Gestão de tipos de descontos</h1>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div
                ...
                <div class="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-800">
                    <h3 class="text-xl font-bold text-slate-900 dark:text-white" id="modalTitle">Criar Novo Desconto</h3>
                ...
                <form class="p-6 space-y-5" action="{{ route('descontos.store') }}" method="POST">
                    @csrf
                    <div>
                        <label for="designacao" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Designação</label>
                        <input id="designacao" name="designacao" class="w-full h-11 px-4 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary focus:border-primary transition-all" placeholder="informe a designação do desconto" type="text" />
                    </div>
                    <div>
                        <label for="desconto" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Taxa de desconto</label>
                        <input id="desconto" name="desconto" class="w-full h-11 px-4 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary focus:border-primary transition-all" placeholder="informe o desconto" type="number" />
                    </div>
                    <div>
                    ...
                    <div class="flex items-center justify-end gap-3 pt-2">
                        ...
                        @if (Auth::user()->can('atribuir desconto'))
...
@section('scripts')
    <script>
                    ...
                    res.data.forEach(s => {
                        rows += `
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors">
                                ...
                                <td class="px-6 py-5">
                                    <p class="font-bold text-slate-500 dark:text-white">${s.desconto}</p>
                                </td>
        ...
        function exportData(documentType) {
            ...
            $.ajax({
                type: "GET",
                url: `/descontos-export`, // rota que vai gerar o arquivo
                data: data,
                xhrFields: {
                ...
                success: function(blob, status, xhr) {
                    ...
                    let filename = documentType === 'excel' ? 'descontos.xlsx' : 'descontos.pdf';
