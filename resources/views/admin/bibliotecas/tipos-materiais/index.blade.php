...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0 text-dark">Tipos de Materiais</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="{{ route('biblioteca.controle') }}">Voltar</a></li>
                    <li class="breadcrumb-item active">Tipos de Materiais</li>
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Cadastrar Tipos de Materiais</h4>
...
<div class="modal fade" id="modalFormEditar">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Editar Tipo Material</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            ...
            <div class="col-12 col-md-12">
                <div class="card">
                    <div class="card-header">
                        <a href="#" class="btn btn-primary float-end" data-toggle="modal" data-target="#modalFormCadastro">Novo Tipo de Material</a>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
...
<script>
    $(function() {
        ...
        $(document).on('click', '.cadastrar', function(e) {
            ...
            $.ajax({
                type: "POST"
                , url: "{{ route('tipos-materiais.store') }}"
                , data: data
                , dataType: "json"
                , beforeSend: function() {
        ...
        $(document).on('click', '.delelte', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        type: "DELETE"
                        , url: `{{ route('tipos-materiais.destroy', ':id') }}`.replace(':id', recordId)
                        , beforeSend: function() {
                            // Você pode adicionar um loader aqui, se necessário
                            progressBeforeSend();
                        }
        ...
        $(document).on('click', '.editar_id', function(e) {
            ...
            $.ajax({
                type: "GET"
                , url: `{{ route('tipos-materiais.edit', ':id') }}`.replace(':id', recordId)
                , beforeSend: function() {
                    // Você pode adicionar um loader aqui, se necessário
                    progressBeforeSend();
                }
        ...
        $(document).on('click', '.updated', function(e) {
            ...
            $.ajax({
                type: "PUT"
                , url: `{{ route('tipos-materiais.update', ':id') }}`.replace(':id', recordId)
                , data: data
                , dataType: "json"
                , beforeSend: function() {
                    // Você pode adicionar um loader aqui, se necessário
