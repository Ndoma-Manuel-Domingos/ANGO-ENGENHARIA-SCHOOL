...
<div class="p-4 lg:p-8 space-y-8">
    <nav class="flex items-center gap-2 text-xs text-slate-500 mb-6">
        ...
        <span class="material-symbols-outlined text-[14px]">chevron_right</span>
        <span class="text-primary font-semibold">gestão de salas</span>
    </nav>
    
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Gestão de Salas</h1>
        ...
        <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex flex-col lg:flex-row gap-4 lg:items-center justify-between">
            <div class="flex flex-wrap gap-3">
                <div class="relative min-w-[240px]">
                    ...
                    <input class="w-full h-10 pl-9 pr-4 text-sm bg-slate-50 dark:bg-slate-800 border-none rounded-lg focus:ring-2 focus:ring-primary" id="sala_designacao" placeholder="Search by name or code..." type="text" />
                </div>
                <select id="sala_status" class="h-10 px-3 bg-slate-50 dark:bg-slate-800 border-none rounded-lg text-sm text-slate-600 dark:text-slate-300 focus:ring-2 focus:ring-primary min-w-[120px]">
                    <option value="">Todos</option>
                    <option value="activo">Activo</option>
                    <option value="desactivo">Desactivo</option>
                </select>
                <select id="sala_registros" class="h-10 px-3 bg-slate-50 dark:bg-slate-800 border-none rounded-lg text-sm text-slate-600 dark:text-slate-300 focus:ring-2 focus:ring-primary min-w-[120px]">
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div
                ...
                <div class="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-800">
                    <h3 class="text-xl font-bold text-slate-900 dark:text-white" id="modalTitle">Criar Nova Sala</h3>
                ...
                <form class="p-6 space-y-5" action="{{ route('salas.store') }}" method="POST">
                    @csrf
                    <div>
                        <label for="salas" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Designação</label>
                        <input id="salas" name="salas" class="w-full h-11 px-4 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary focus:border-primary transition-all" placeholder="informe a designação da sala" type="text" />
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="col-span-2 md:col-span-1">
                            <label for="tipo" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Tipo de Sala</label>
                            <input id="tipo" name="tipo" class="w-full h-11 px-4 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary focus:border-primary transition-all" placeholder="e.g. MATH-402" type="text" />
                        </div>
                    </div>
                    <input type="hidden" id="sala_id" name="sala_id">
                    <div>
                        <label for="descricao" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Description</label>
...
@section('scripts')
    <script>
        ...
        $("#sala_designacao").on('input', function () {
            load(1);
        });
        
        $("#sala_status").change(function(){
            load(1);
        });
        
        $("#sala_registros").change(function(){
            load(1);
        });
        ...
        function load(page=1){
            $.ajax({
                ...
                data: {
                    page: page,
                    sala_designacao: $("#sala_designacao").val(),
                    sala_status: $("#sala_status").val(),
                    sala_registros: $("#sala_registros").val(),
                },
                dataType: "json",
                beforeSend: function () {
                            ...
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors">
                                <td class="px-6 py-5">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="font-bold text-slate-500 dark:text-white">${s.salas}</p>
                                            <p class="text-xs text-slate-500">${s.tipo}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-5">
                                ...
                                <td class="px-6 py-5">
                                    <div class="flex justify-end gap-1">
                                        <button class="p-2 text-slate-400 hover:text-primary hover:bg-primary/5 rounded-lg transition-all" title="View">
        ...
        $(document).ready(function() {
            $('form').on('submit', function(e) {
                ...
                if ($("#sala_id").val() == null || $("#sala_id").val() == "") {
                    new_form = form.attr('action');
                    method = "post";
                } else {
                    method = "put";
                    new_form = form.attr('action') + "/" + $("#sala_id").val();
                }
        ...
        function edit(id) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("#sala_id").val(data.id);
                    $("#salas").val(data.salas);
                    $("#status").val(data.status);
                    $("#tipo").val(data.tipo);
                    $("#descricao").val(data.descricao);
        
                    $("#modalTitle").text("Editar Sala");
                    $("#botao_submiter").text("Editar Sala");
                ...
                error: function (xhr) {
                    ...
                    let message = "Erro ao carregar dados da sala";
        ...
        function exportData(documentType) {
            ...
            $.ajax({
                type: "GET",
                url: `/salas-export`, // rota que vai gerar o arquivo
                data: data,
                xhrFields: {
                ...
                success: function(blob, status, xhr) {
                    ...
                    let filename = documentType === 'excel' ? 'salas.xlsx' : 'salas.pdf';
