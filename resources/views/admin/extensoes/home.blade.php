...
<div class="p-4 lg:p-8 space-y-8">
    <nav class="flex items-center gap-2 text-xs text-slate-500 mb-6">
        ...
        <span class="material-symbols-outlined text-[14px]">chevron_right</span>
        <span class="text-primary font-semibold">gestão de extensões</span>
    </nav>

    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Gestão de extensões</h1>
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-slate-800/30 border-b border-slate-100 dark:border-slate-800">
                        ...
                        <th class="px-6 py-4 text-xs font-bold uppercase text-slate-500 tracking-wider">Sufixo</th>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-white dark:bg-sidebar-dark w-full max-w-xl rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                <div class="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-800">
                    <h3 class="text-xl font-bold text-slate-900 dark:text-white" id="modalTitle">Criar Nova Extensão</h3>
                ...
                <form class="p-6 space-y-5" action="{{ route('extensao.store') }}" method="POST">
                    @csrf
                    <div>
                    ...
                    <div>
                        <label for="sufix" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Sufixo</label>
                        <input id="sufix" name="sufix" class="w-full h-11 px-4 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary focus:border-primary transition-all" placeholder="Informe o sufixo" type="text" />
                    </div>
                    <div>
                        <div class="col-span-2 md:col-span-1">
                            ...
                            <select id="tipo" name="tipo" class="w-full h-11 px-4 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary focus:border-primary transition-all">
                                ...
                                <option value="estudantes">Processo Estudantes</option>
                                <option value="funcionarios">Processo Funcionários</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-3 pt-2">
                        ...
                        @if (Auth::user()->can('create: escolaridade'))
...
<script>
                          ...
                          <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors">
                              <td class="px-6 py-5">
                                  <div class="flex items-center gap-3">
                                      ...
                                      <div>
                                          <p class="font-bold text-slate-500 dark:text-white">${s.extensao}</p>
                              ...
                              <td class="px-6 py-5">
                                ...
                                <td class="px-6 py-5">
                                    <p class="font-bold text-slate-500 dark:text-white">${s.sufix}</p>
                                </td>
                                <td class="px-6 py-5">
    ...
    $(document).on('click', '.delete-record', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: `{{ route('extensao.destroy', ':id') }}`.replace(':id', recordId)
                    , method: 'DELETE'
                    , data: {
    ...
    function edit(id) {
        $.ajax({
            ...
            , success: function(data) {
                ...
                $("#data_id").val(data.id);
                $("#designacao").val(data.extensao);
                $("#status").val(data.status);
                $("#sufix").val(data.sufix);
                $("#tipo").val(data.tipo);

                $("#modalTitle").text("Editar Extensão");
                $("#botao_submiter").text("Editar Extensão");
            ...
            , error: function(xhr) {
                ...
                let message = "Erro ao carregar dados da Extensão";
    ...
    function exportData(documentType) {
        ...
        $.ajax({
            type: "GET"
            , url: `/extensao-export`, // rota que vai gerar o arquivo
            data: data
            , xhrFields: {
            ...
            , success: function(blob, status, xhr) {
                ...
                let filename = documentType === 'excel' ? 'extensao.xlsx' : 'extensao.pdf';
