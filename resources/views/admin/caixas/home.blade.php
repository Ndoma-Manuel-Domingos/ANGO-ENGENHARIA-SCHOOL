...
<div class="p-4 lg:p-8 space-y-8">
    <nav class="flex items-center gap-2 text-xs text-slate-500 mb-6">
        ...
        <span class="material-symbols-outlined text-[14px]">chevron_right</span>
        <span class="text-primary font-semibold">gestão de caixas</span>
    </nav>
    
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Gestão de caixas</h1>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div
                ...
                <div class="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-800">
                    <h3 class="text-xl font-bold text-slate-900 dark:text-white" id="modalTitle">Criar Nova caixa</h3>
                ...
                <form class="p-6 space-y-5" action="{{ route('caixas.store') }}" method="POST">
                    @csrf
                    <div>
                        <label for="designacao" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Designação</label>
                        <input id="designacao" name="designacao" class="w-full h-11 px-4 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary focus:border-primary transition-all" placeholder="informe a designação do caixa" type="text" />
                    </div>
                    <div>
...
@section('scripts')
    <script>
                            ...
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors">
                                <td class="px-6 py-5">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="font-bold text-slate-500 dark:text-white">${s.caixa}</p>
        ...
        function edit(id) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("#data_id").val(data.id);
                    $("#designacao").val(data.caixa);
                    $("#status").val(data.status);
        
                    $("#modalTitle").text("Editar Caixa");
                    $("#botao_submiter").text("Editar Caixa");
                ...
                error: function (xhr) {
                    ...
                    let message = "Erro ao carregar dados da caixa";
        ...
        function exportData(documentType) {
            ...
            $.ajax({
                type: "GET",
                url: `/caixas-export`, // rota que vai gerar o arquivo
                data: data,
                xhrFields: {
                ...
                success: function(blob, status, xhr) {
                    ...
                    let filename = documentType === 'excel' ? 'caixas.xlsx' : 'caixas.pdf';
