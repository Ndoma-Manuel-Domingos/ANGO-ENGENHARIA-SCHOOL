...
<div class="p-4 lg:p-8 space-y-8">
    <nav class="flex items-center gap-2 text-xs text-slate-500 mb-6">
        ...
        <span class="material-symbols-outlined text-[14px]">chevron_right</span>
        <span class="text-primary font-semibold">gestão de bancos</span>
    </nav>
    
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Gestão de bancos</h1>
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-slate-800/30 border-b border-slate-100 dark:border-slate-800">
                        ...
                        <th class="px-6 py-4 text-xs font-bold uppercase text-slate-500 tracking-wider">Conta</th>
                        <th class="px-6 py-4 text-xs font-bold uppercase text-slate-500 tracking-wider">Número da Conta</th>
                        <th class="px-6 py-4 text-xs font-bold uppercase text-slate-500 tracking-wider">IBAN</th>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div
                ...
                <div class="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-800">
                    <h3 class="text-xl font-bold text-slate-900 dark:text-white" id="modalTitle">Criar Novo banco</h3>
                ...
                <form class="p-6 space-y-5" action="{{ route('bancos.store') }}" method="POST">
                    @csrf
                    <div>
                        <label for="designacao" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Designação</label>
                        <input id="designacao" name="designacao" class="w-full h-11 px-4 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary focus:border-primary transition-all" placeholder="informe a designação do banco" type="text" />
                    </div>
                    <div>
                        <label for="numero_conta" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Nº Conta Bancária</label>
                        <input id="numero_conta" name="numero_conta" class="w-full h-11 px-4 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary focus:border-primary transition-all" placeholder="informe o número da conta" type="text" />
                    </div>
                    <div>
                        <label for="iban" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Nº IBAN</label>
                        <input id="iban" name="iban" class="w-full h-11 px-4 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary focus:border-primary transition-all" placeholder="informe o IBAN" type="text" />
                    </div>
                    
                    <div>
                        <div class="col-span-2 md:col-span-1">
                            <label for="conta" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Tipo de conta</label>
                            <select id="conta" name="conta" class="w-full h-11 px-4 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-primary focus:border-primary transition-all">
                                <option value="42.1.">42.1 - Depósitos a prazo</option>
                                <option value="43.1.">43.1 - Depósitos à Ordem</option>
                                <option value="44.1.">44.1 - Outros Depósitos</option>
                            </select>
                        </div>
                    </div>
...
@section('scripts')
    <script>
                            ...
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors">
                                <td class="px-6 py-5">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="font-bold text-slate-500 dark:text-white">${s.banco}</p>
                                            <p class="text-xs text-slate-500">${s.conta}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-5">
                                ...
                                <td class="px-6 py-5">
                                    <p class="font-bold text-slate-500 dark:text-white">${s.conta}</p>
                                </td>
                             
                                <td class="px-6 py-5">
                                    <p class="font-bold text-slate-500 dark:text-white">${s.numero_conta}</p>
                                </td>
                             
                                <td class="px-6 py-5">
                                    <p class="font-bold text-slate-500 dark:text-white">${s.iban}</p>
                                </td>
        ...
        $(document).on('click', '.delete-record', function(e) {
            ...
            , }).then((result) => {
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: `{{ route('bancos.destroy', ':id') }}`.replace(':id', recordId), 
                        method: 'DELETE', 
                        data: {
        ...
        function edit(id) {
            $.ajax({
                ...
                error: function (xhr) {
                    ...
                    let message = "Erro ao carregar dados da banco";
        ...
        function exportData(documentType) {
            ...
            $.ajax({
                type: "GET",
                url: `/bancos-export`, // rota que vai gerar o arquivo
                data: data,
                xhrFields: {
                ...
                success: function(blob, status, xhr) {
                    ...
                    let filename = documentType === 'excel' ? 'bancos.xlsx' : 'bancos.pdf';
