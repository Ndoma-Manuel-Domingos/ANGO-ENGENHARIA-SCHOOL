<?php

namespace App\Http\Controllers;

use App\Models\Shcool;
use App\Models\User;
use App\Models\web\biblioteca\Editora;
use Illuminate\Support\Facades\Validator;
use Illuminate\Http\Request;

use Illuminate\Support\Facades\Auth;

class EditoraController extends Controller
{
    //
    use TraitHelpers;
    
    public function __construct()
    {
        $this->middleware('auth');
    }

    // view cursos principal
    public function index()
    {
        $user = auth()->user();
        
        // if(!$user->can('read: curso')){
        //     Alert::error('Acesso restrito', 'Você não possui permissão para esta operação, por favor, contacte o administrador!');
        //     return redirect()->back();
        // }
    
        
        
        $headers = [
            "escola" => Shcool::with('ensino')->findOrFail($this->escolarLogada()),
            "titulo" => "Listagem de Generos para livros",
            
            "descricao" => env('APP_NAME'),
            "usuario" => User::findOrFail(Auth::user()->id),
            "editoras" => Editora::where('shcools_id', $this->escolarLogada())->get(),
        ];
        
        return view('admin.bibliotecas.editoras.index', $headers);
    }

    // cadastrar cursos
    public function store(Request $request)
    {
        $user = auth()->user();
        
        // if(!$user->can('create: curso')){
        //     Alert::error('Acesso restrito', 'Você não possui permissão para esta operação, por favor, contacte o administrador!');
        //     return redirect()->back();
        // }
    
        $validate = Validator::make($request->all(), [
            "nome" => 'required',
            "nif" => 'required',
        ], [
            "nome.required" => "Campo Obrigatório",
            "nif.required" => "Campo Obrigatório",
        ]);

        if ($validate->fails()) {
            return response()->json([
                'status' => 400,
                'errors' => $validate->messages(),
            ]);
        }else{
            $create = Editora::create([
                'nome' => $request->nome,
                'nif' => $request->nif,
                // 'pais' => $request->pais,
                'ano_fundacao' => $request->ano_fundacao,
                'website' => $request->website,
                'email' => $request->email,
                'telefone' => $request->telefone,
                'endereco' => $request->endereco,
                'observacao' => $request->observacao,
                'shcools_id' => $this->escolarLogada(),
            ]);

            return response()->json([
                'status' => 200,
                'message' => 'Dados salvos com sucesso!',
                "usuario" => User::findOrFail(Auth::user()->id),
            ]);
        }
    }

    // editar cursos
    public function edit($id)
    {
    
        $user = auth()->user();
        
        // if(!$user->can('update: curso')){
        //     Alert::error('Acesso restrito', 'Você não possui permissão para esta operação, por favor, contacte o administrador!');
        //     return redirect()->back();
        // }
        
        $registro = Editora::findOrFail($id);

        if ($registro) {
            return response()->json([
                "status" => 200,
                "registro" => $registro,
                "usuario" => User::findOrFail(Auth::user()->id),
            ]);
        }else{
            return response()->json([
                "status" => 404,
                "message" => 'Turno não Encontrado',
            ]);
        }

    }

    // actualizar cursos
    public function update(Request $request, $id)
    {
        $user = auth()->user();
        
        // if(!$user->can('update: curso')){
        //     Alert::error('Acesso restrito', 'Você não possui permissão para esta operação, por favor, contacte o administrador!');
        //     return redirect()->back();
        // }
        
        $validate = Validator::make($request->all(), [
            "nome" => 'required',
            "nif" => 'required',
        ], [
            "nome.required" => "Campo Obrigatório",
            "nif.required" => "Campo Obrigatório",
        ]);

        if ($validate->fails()) {
            return response()->json([
                'status' => 400,
                'errors' => $validate->messages(),
            ]);
        }else{
            $update = Editora::findOrFail($id);

            if ($update) {
                $update->nome = $request->nome;
                $update->nif = $request->nif;
                // $update->pais = $request->pais;
                $update->ano_fundacao = $request->ano_fundacao;
                $update->website = $request->website;
                $update->email = $request->email;
                $update->telefone = $request->telefone;
                $update->endereco = $request->endereco;
                $update->observacao = $request->observacao;
                $update->update();

                return response()->json([
                    'status' => 200,
                    'message' => 'Dados ACtualizados com sucesso!',
                    "usuario" => User::findOrFail(Auth::user()->id),
                ]);
            }else{
                return response()->json([
                    "status" => 404,
                    "message" => 'Turno não Encontrado'
                ]);
            }
        }
    }

    // delete cursos
    public function destroy($id)
    {
        $user = auth()->user();
        
        // if(!$user->can('delete: curso')){
        //     Alert::error('Acesso restrito', 'Você não possui permissão para esta operação, por favor, contacte o administrador!');
        //     return redirect()->back();
        // }
        
        $delete = Editora::findOrFail($id);
        $delete->delete();

        return response()->json([
            'status' => 200,
            'message' => 'Dados Excluido com sucesso!',
            "usuario" => User::findOrFail(Auth::user()->id),
        ]);
    }
  
}
