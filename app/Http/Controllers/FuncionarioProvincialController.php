...
class FuncionarioProvincialController extends Controller
{
    use TraitHelpers;
    ...
    public function FuncionariosControlo(Request $request)
    {
        ...
        $result = Funcionarios::selectRaw('
            COUNT(*) as total,
            COUNT(CASE WHEN genero = "Masculino" THEN 1 END) as masculino,
            COUNT(CASE WHEN genero = "Femenino" THEN 1 END) as femenino,
            COUNT(CASE WHEN status = "activo" THEN 1 END) as funcionario_activos,
            COUNT(CASE WHEN status = "desactivo" THEN 1 END) as funcionario_desactivos,
            COUNT(CASE WHEN genero = "Masculino" AND status = "activo" THEN 1 END) as funcionario_activos_masculinos,
            COUNT(CASE WHEN genero = "Femenino" AND status = "activo" THEN 1 END) as funcionario_activos_femeninos,
            COUNT(CASE WHEN genero = "Masculino" AND status = "desactivo" THEN 1 END) as funcionario_desactivos_masculinos,
            COUNT(CASE WHEN genero = "Femenino" AND status = "desactivo" THEN 1 END) as funcionario_desactivos_femeninos
        ')
        ->where('level', '2')
        ->first();
        ...
        $data->addStringColumn('Gênero / Status')
             ->addNumberColumn('Quantidade')
             ->addRow(['Masculino', $result->masculino])
             ->addRow(['Feminino', $result->femenino])
             ->addRow(['Ativos', $result->funcionario_activos])
             ->addRow(['Inativos', $result->funcionario_desactivos])
             ->addRow(['Ativos Masculinos', $result->funcionario_activos_masculinos])
             ->addRow(['Ativos Femeninos', $result->funcionario_activos_femeninos])
             ->addRow(['Inativos Masculinos', $result->funcionario_desactivos_masculinos])
             ->addRow(['Inativos Femininos', $result->funcionario_desactivos_femeninos])
             ->addRow(['Total', $result->total]);
             
        $options = [
            'title' => 'Estatísticas de Funcionários',
            'height' => 290,
            'colors' => ['orange'],
            'titleTextStyle' => [
                'color'    => 'rgb(123, 65, 89)',
        ...
        if ($request->tipo_graficos == 'ColumnChart'){
            $chart = $lava->ColumnChart('Funcionarios', $data, $options);
        }else
        
        if ($request->tipo_graficos == 'BarChart'){
            $chart = $lava->BarChart('Funcionarios', $data, $options);
        }else
        
        if ($request->tipo_graficos == 'AreaChart'){
            $chart = $lava->AreaChart('Funcionarios', $data, $options);
        }else
        
        if ($request->tipo_graficos == 'DonutChart'){
            $chart = $lava->DonutChart('Funcionarios', $data, $options);
        }else
        
        if ($request->tipo_graficos == 'PieChart'){
            $chart = $lava->PieChart('Funcionarios', $data, $options);
        }else
        
        if ($request->tipo_graficos == 'LineChart'){
            $chart = $lava->LineChart('Funcionarios', $data, $options);
        }else {
            $chart = $lava->ColumnChart('Funcionarios', $data, $options);
        }
        ...
        $headers = [ 
            ...
            "lava" => $lava,
            "filtros" => $request->all("tipo_graficos"),
        ];
        
        return view('sistema.direccao-provincial.funcionarios.controlo', $headers);
    }
    ...
    public function Funcionarios(Request $request)
    {
        ...
        return view('sistema.direccao-provincial.funcionarios.funcionarios', $headers);
    }
    ...
    public function FuncionariosCreate()
    {
        ...
        return view('sistema.direccao-provincial.funcionarios.create', $headers);
    }
    ...
    public function FuncionariosDepartamento(Request $request)
    {
        ...
        return view('sistema.direccao-provincial.funcionarios.funcionarios-departamentos', $headers);
    }
    ...
    public function FuncionariosCargo(Request $request)
    {
        ...
        return view('sistema.direccao-provincial.funcionarios.funcionarios-cargos', $headers);
    }
    ...
    public function FuncionariosShow($id)
    {
        ...
        return view('sistema.direccao-provincial.funcionarios.show', $headers);
    }
    ...
    public function FuncionariosDuplicar($id)
    {
        $user = auth()->user();
        ...
        $academico = FuncionariosAcademico::where('funcionarios_id', $funcionario->id)->first();
        $arquivo = Arquivo::where('level', '2')->where('model_id', $funcionario->id)->first();
        ...
        return view('sistema.direccao-provincial.funcionarios.duplicar', $headers);
    }

    public function FuncionariosDuplicarStore(Request $request)
    {
        $request->validate([
            'nome' => 'required',
            'sobre_nome' => 'required',
        ...
        if($meses){
            foreach($meses as $mes){
                ...
                if(!$verificar){
                    ...
                    $newCreate->shcools_id = $id_instituicao->id;	
        ...
        Alert::success('Bom Trabalho', 'Dados Duplicados com sucesso!');
        return redirect()->back();
    }
    ...
    public function FuncionariosEdit($id)
    {
        ...
        return view('sistema.direccao-provincial.funcionarios.edit', $headers);
    }
    ...
    public function FuncionariosUpdate(Request $request, $id)
    {
        ...
        $update->outras_redes = $request->outras_redes;
        $update->level = $level;
        $update->shcools_id = $id_instituicao->id;
        ...
        $udpateAcademico->shcools_id = $id_instituicao->id;
        ...
        if($contrato){
            ...
            $contrato->shcools_id = $id_instituicao->id;
            ...
            $create3->data_at = date("Y-m-d");
            $create3->ano_lectivos_id = NULL;
            $create3->shcools_id = $id_instituicao->id;
            $create3->save();
    ...
    public function FuncionariosDestroy($id)
    {
        ...
        $funcionario = Funcionarios::findOrFail(Crypt::decrypt($id));
        $academico = FuncionariosAcademico::where('funcionarios_id', $funcionario->id)->first();
        $arquivo = Arquivo::where('level', '2')->where('model_type', 'funcianario')->where('model_id', $funcionario->id)->first();
