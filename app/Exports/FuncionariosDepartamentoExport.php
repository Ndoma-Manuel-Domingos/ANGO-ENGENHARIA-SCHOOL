...
class FuncionariosDepartamentoExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithDrawings, WithCustomStartCell, WithStyles, WithTitle
{
    use TraitHelpers;
    
    public $instituicao, $tempo_trabalho, $genero, $status, $departamento_id;

    public function __construct($instituicao, $tempo_trabalho, $genero, $status, $departamento_id)
    {
    ...
    public function collection()
    {
        ...
        ->whereHas('contrato', function($query) use ($id, $instituicao){
            $query->when($id, function ($query) use ($id, $instituicao){
                $query->where('departamento_id', $id)->where('level', $instituicao);
            });
        })
